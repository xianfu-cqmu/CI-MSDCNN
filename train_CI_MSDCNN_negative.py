...
class SparseSpectralDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = self.samples[cas_no]  # 原始 B 模型：形状应为 [1, 104000] 或 [104000,]
...
#
# 保持程序 B 原来的 RTWeightedLoss 逻辑：
# 1. 先计算基础 BCE loss
# 2. 计算 batch 内样本两两之间的属性相似性矩阵
# 3. 根据相似性计算权重
# 4. base_loss * weights
# 5. 最后 mean
#
...
class RTCCSpKaWeightedLoss(nn.Module):
    def __init__(
        ...
        # property_weights 对应 [RT, predicted CCS, predicted basic pKa]，本模型使用 0:1:3
    ...
    def forward(self, preds, targets, physchems):
        ...
        # 计算 RT / predicted CCS / predicted basic pKa 联合相似性矩阵
        physchem_similarity = self.calculate_physchem_similarity(physchems)

        # 根据联合相似性调整损失权重
        # 保持程序 B 原来的形式：
        # weights = torch.exp(-similarity / tolerance)
        weights = torch.exp(
    ...
    def calculate_physchem_similarity(self, physchems):
        ...
        # 计算属性差异矩阵
        # physchem_diff: [batch_size, batch_size, 3]
        physchem_diff = torch.abs(
            physchems.unsqueeze(1) - physchems.unsqueeze(0)
        )

        # 将差异转换为相似性
        # physchem_similarity_each: [batch_size, batch_size, 3]
        ...
        # ========================
        # 关键修复：
        # 确保 property_weights 和 physchems 在同一个 device
        # ========================
        property_weights = self.property_weights.to(physchems.device)

        # 三个属性加权融合
        # physchem_similarity: [batch_size, batch_size]
        physchem_similarity = torch.sum(
            physchem_similarity_each * property_weights.view(1, 1, -1),
            dim=-1
        )
...
if __name__ == '__main__':
    ...
    # 该 Excel 中需要包含 RT、predicted CCS、predicted basic pKa 三列
    ...
    # 使用全部负离子样本进行训练。
    # 不从训练数据中划分 300 个样本作为内部验证集；
    # 模型性能由单独、独立的验证集在训练完成后进行评估。
    train_set = dataset
    ...
    # 使用 RT + predicted CCS + predicted basic pKa（0:1:3）联合约束的 BCE 损失
    ...
    criterion = RTCCSpKaWeightedLoss(
        ...
        similarity_scale=1.0,
        property_weights=[0.0, 1.0, 3.0]
        # 对应 [RT, predicted CCS, predicted basic pKa]，比例 0:1:3
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y, physchem, _) in enumerate(train_loader):
            ...
            # RT + predicted CCS + predicted basic pKa（0:1:3）联合约束
        ...
        if (epoch + 1) == epochs:
            model_path = (
                f'../Result/models/'
                f'2026C18negRT_pCCS_pKa_0to1to3_premodel_epoch_{epoch + 1}.pth'
            )
