...
class SparseSpectralDataset(Dataset):
    ...
    def __init__(self, h5_file, label_file):
        ...
        # The source table is expected to contain these columns.
        ...
        # Keep samples that have both spectral data and complete physicochemical metadata.
        self.cas_nos = [
            key for key in self.h5_file.keys()
            if key in self.physchem_values and key in self.labels
        ]
...
class RTCCSpKaWeightedLoss(nn.Module):
    ...
    def __init__(
        ...
        # Relative weights correspond to [RT, predicted CCS, predicted basic pKa].
...
def main():
    ...
    # Use the complete positive-ion training dataset.
    ...
    # CI-CNN uses the chemistry-informed constraint with
    # relative weights [RT, predicted CCS, predicted basic pKa] = [1, 2, 0].
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (x, y, physchem, _) in enumerate(train_loader):
            ...
            if i == 0 and epoch == 0:
                ...
                print(
                    f"Physicochemical columns: "
                    f"{dataset.physchem_cols}"
                )
